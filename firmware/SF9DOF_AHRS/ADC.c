//ADC variables
static volatile uint8_t MuxSel = 0;
static volatile uint8_t analog_reference;
static volatile uint16_t analog_buffer[8];
static volatile uint8_t analog_count[8];
...
void Read_adc_raw(void)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (temp2 > 0)
			AN[i] = (float) temp1 / (float) temp2; // Check for divide by zero
